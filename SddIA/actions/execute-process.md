...
## 1. PropÃ³sito
Orquestar de extremo a extremo la ejecuciÃ³n autÃ³noma de un **proceso** del Core SddIA: resolver su ley vÃ­a CÃºmulo, recorrer sus fases con Tekton, someter cada intenciÃ³n ejecutiva a Cerbero y delegar la fÃ­sica exclusivamente en **skills**, **tools** o **actions** autorizados. A partir de `process-contract v1.2.0`, las fases pueden declarar **`phase_invocations`**: secuencias ejecutables con stdin JSON, bindings de salida y reglas de error. Con `process-contract v1.3.0`, la resoluciÃ³n de identidad incluye **aliases** gobernados por CÃºmulo.

## 2. OrquestaciÃ³n lÃ³gica (sin terminal nativa)

1. **Carga de ley (CÃºmulo):** Tekton solicita al agente **CÃºmulo** (o al manifiesto SSOT `cumulo.paths.json` inyectado en su `cumulo_topology`) la ruta canÃ³nica de `directories.process`. **Antes** de componer la ruta fÃ­sica `{...}.md`, CÃºmulo debe resolver el `process_name` recibido contra el **mapa de identidad** del catÃ¡logo de procesos (nombres canÃ³nicos `name` / fichero + `aliases` opcionales del frontmatter), derivado en runtime segÃºn Â§5 de `agents/cumulo.md`. La ruta final debe ser siempre `{directorio_process}/{nombre_canÃ³nico}.md` (nombre de fichero soberano). Si el input coincide con un canÃ³nico y con un alias ambiguo, **gana el canÃ³nico** (`process-contract v1.3.0` Â§1.1). Si tras la resoluciÃ³n el archivo no existe o el mapa es invÃ¡lido, emitir `status_code: 1` y detener.
...
* **Contrato de process:** `paths.contracts.process` â€” schema `phase_invocations` v1.2.0+; identidad y `aliases` v1.3.0.
