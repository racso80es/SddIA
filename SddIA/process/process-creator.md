...
outputs:
  - "artifact_process_md": "Archivo `SddIA/process/{process_name}.md` generado con cabecera YAML conforme a contrato"
  - "artifact_process_index": "`SddIA/process/index.md` actualizado con fila alineada a la cabecera del nuevo proceso"
phases:
...
1. Verificar que `process_name` sea único en `SddIA/process` y cumpla kebab-case.
...
3. Escribir `SddIA/process/{process_name}.md` con cabecera YAML (uuid, name, version, contract, context, hash_signature, inputs, phases, outputs, métricas si aplican) y cuerpo que describa cada fase en prosa operativa.
...
1. Abrir `SddIA/process/index.md` y comprobar encabezados de tabla exigidos por Gobernanza de Índices.
