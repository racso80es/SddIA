...
| **capsule-json-io.md** | Envelope JSON compartido (stdin/stdout) para invocar skills y tools desde agentes; SSOT de meta, request y result. |
| **entidades-dominio-ecosistema-sddia.md** | Definición de entidades de dominio (ecosistema SddIA): las que integran el contrato/ítem Token; obligación de estructura (archivo .md con frontmatter YAML); tokens bajo `paths.tokensPath` según `tokens-contract.md` y `<token-id>.md`. |
