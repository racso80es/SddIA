#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Regresión forja ECST fractal — event_family explícito (Kaizen)."""
...
from execute_process_capsules import resolve_event_family_required, run_event_forge
...
class TestEventForgeFractal(unittest.TestCase):
    def test_missing_event_family_raises(self) -> None:
        with self.assertRaisesRegex(ValueError, "event_family es obligatorio"):
            resolve_event_family_required({})

    def test_invalid_event_family_raises(self) -> None:
        with self.assertRaisesRegex(ValueError, "event_family inválido"):
            resolve_event_family_required({"event_family": "legacy"})
