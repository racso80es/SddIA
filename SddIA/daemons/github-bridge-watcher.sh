...
if BIN="$(_sddia_resolve_daemon_binary "$REPO_ROOT" github-bridge-watcher)"; then
  exec "$BIN" "$@"
fi

echo "[github-bridge-watcher] binario no encontrado (build: cd SddIA && cargo build -p github-bridge-watcher)" >&2
exit 1
