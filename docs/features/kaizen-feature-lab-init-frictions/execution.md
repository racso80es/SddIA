...
| T7 | en curso | validacion APTO + PBI a `done/` + un PR |
...
| Forja `entity-manager` idempotente sin `inputs` explícitos | Primer intento no movió el sello y dejó residuo en `eda-coverage.json` | `patch_process_phases_update` acepta `inputs`; residuo purgado |
