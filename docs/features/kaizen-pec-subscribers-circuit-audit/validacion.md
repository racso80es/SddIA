...
scope: "PPR Cosecha Kaizen — kaizen-pec-subscribers-circuit-audit (PR #181 · ECST DLKDvjJ7…)"
authorization_status:
  exitCode: 0
  signer_identity_rbac: Vertice_Biologico_Relay
  emitter_agent: github-bridge-watcher
  note: "KAIZEN_COSECHA_GATE APTO · kaizen_seeds 0 · dedup 2 (#177 DCC sibling + #136 Shell) · F5 heredado APTO · MERGE ausente → accept_pr_handoff true · Shell git-manager Rejected — sin stdout inventado"
...
evidence_bridge_notes: "R1/R2 copia Runtime evidence (Argos F5 06:16:30Z CID DLKDvjJ7) source=native_state notes=idempotent-hit; TECH_FORMAL_* / GIT_EVIDENCE_VIA_GIT_MANAGER APTO; gemelo prosthesis_subprocess @ 2026-08-17T05:57:09Z formal_evidence_detail=verify-process-integrity: OK; Shell git-manager Rejected esta sesión Cúmulo — sin stdout inventado"
shell_git_manager_session: "Rejected — sin gitStdout físico esta invocación Cúmulo Cosecha CID DLKDvjJ7…"
checks:
...
situational_notes:
  - "delivery-close-cycle ∈ revoked since 2026-08-16T16:40:55Z — fuera E1 este CID (emisor=github-bridge-watcher); sighting sibling Presented 94b7f03c… → dedup #177"
...
**APTO** — `resolution: KAIZEN_COSECHA_GATE` · `kaizen_seeds: 0` · `kaizen_seeds_dedup: 2` · `delivery_state: success` (heredado F5) · `accept_pr_handoff: true` (merge no observado).

| Gate | Estado | Criterio |
|------|--------|----------|
| F5 (heredado) | **APTO** | `PASS_F5_VERDICT` · CID `DLKDvjJ7…` · `verdict: aprobado` |
| Cosecha | **APTO** | 0 seed nueva + 2 dedup; sin DIA alert |
| KM RBAC | **APTO** | solo Cúmulo escribe `docs/todos/` esta fase (sightings dedup) |
| Merge | **NO_APTO** | sin `PullRequest_Merged` para `DLKDvjJ7…` / PR #181 → handoff `accept-pr` procede |
...
| `materialized_at` (machine ref) | `2026-08-17T06:09:47Z` (Cerbero F4) · Argos F5 `2026-08-17T06:16:30Z` |
| `GIT_EVIDENCE_SESSION_SHELL` | **NO_APTO** — `./sddia-run.sh --tool git-manager` → Shell Rejected; sin `gitStdout` físico esta sesión Cúmulo |

Bloque machine de referencia: `_agent_handoff.md` schema `kalma2-agent-runtime-evidence/v1` (Cerbero F4 CID `DLKDvjJ7…` @ `2026-08-17T06:09:47Z`; Argos F5 `06:16:30Z`).

## Cosecha — inventario de deuda

| Hallazgo (F5/F4) | Acción Cúmulo | Destino |
|------------------|---------------|---------|
| Sibling DCC `RBAC_EMITTER_NOT_REVOKED` (`94b7f03c…`) | **dedup** | done `[ARQUITECTURA] … (PPR #177)` / canónico olas · DCC∈revoked `since 2026-08-16T16:40:55Z`; sighting PR #181 · harvest CID `DLKDvjJ7…` |
| `GIT_EVIDENCE_SESSION_SHELL` / `F3_TECH_GATE` | **dedup** | done `[OPERATIVO] … (PPR #136)` residual Kalma2 Shell/git-manager |
| `RBAC_EMITTER_NOT_REVOKED` (este CID) | no seed | emisor `github-bridge-watcher` ∉ revoked → **APTO** |
...
| FIX watcher GBW / telegram | no seed | pending `System_Fracture_Detected` `f34e42b10828` / `4d9431bc66b3`; fuera `document_id` |
...
| ref local (FS) | `0d177538…` |
| Merged (este CID / PR #181) | **ausente** |
| `delivery-close-cycle` revoked | since `2026-08-16T16:40:55Z` · `success_rate_below_threshold` · `entity_type: tool` (sibling; fuera E1 este CID) |
| `github-bridge-watcher` revoked | **ausente** |
...
```text
aprobado — KAIZEN_COSECHA_GATE · kaizen_seeds 0 · dedup 2 (#177 DCC sibling + #136 Shell);
F5 heredado success · accept_pr_handoff true (sin PullRequest_Merged DLKDvjJ7 / PR #181);
PBI archivado en done/; sin Kaizen_Alert_Required; R1/R2 APTO vía Evidence Bridge native_state;
GIT_EVIDENCE_SESSION_SHELL NO_APTO (Shell Rejected; sin stdout inventado); CID DLKDvjJ7… · sibling 94b7f03c….
```
