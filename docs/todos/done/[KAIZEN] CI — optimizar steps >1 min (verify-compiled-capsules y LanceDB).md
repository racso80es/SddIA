...
- [x] **CA1 (Umbral, anti-maquillaje):** SHA-2 run 33506016317: Build+verify **160 s** (techo 170 s), LanceDB **92 s** (techo 180 s).
- [x] **CA2 (Cobertura):** el gate sigue exigiendo el mismo conjunto de bins `main.rs` (hoy 29 + `mandatory_bins`). Los 16 tests actuales (o superconjunto / mapeo explícito si hay integración) se ejecutan.
- [x] **CA3 (Inventario):** A0 del run `33477170741` queda como SSOT en este PBI. El PR de cierre adjunta tabla de steps del mismo workflow; wasi cache 94 s **diferido** (fuera de A3.2).
- [x] **CA4 (Cache):** A3.2 cablea `SCCACHE_GHA_ENABLED` (job) + `RUSTC_WRAPPER` (post-action). SHA-1 26 % hits; SHA-2 88 % hits; location ghac. `SddIA/target` fuera del blob.
- [x] **CA5 (Job):** wall-clock integrity **277 s (4 m 37 s)** < 8 min en SHA-2 `pull_request`.
- [x] **CA6 (Hermandad):** `wasi-runtime-smoke`, `eda-bus-e2e-smoke`, `eda-iota-smoke-simulate` verdes en el PR de cierre. `eda-iota-physical` con `conclusion: success`; si el secret está ausente, el skip `exit 0` no cuenta como prueba de anclaje (fuera de alcance de este PBI).
- [x] **CA7 (Forja):** si se muta `SddIA/tools/`, vía `./sddia-run.sh --process entity-manager`. Workflow `.github/` en el mismo PR. Evolution anclado a `530039c9-100b-413a-b3d5-ca632d83acc6`.
