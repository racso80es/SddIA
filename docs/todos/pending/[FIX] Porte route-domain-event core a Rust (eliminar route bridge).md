...
P17 cerró el entrypoint orquestador binario-only. La lógica ECST/fan-out de enrutamiento EDA sigue en Python vía `_execute_process_route_bridge.py` → `route_domain_event_core.py`.
...
- Golden `route-domain-event` verde sin bridge Python.
- Smoke `eda-e2e-lab` 8/8.
- Eliminar `_execute_process_route_bridge.py` y referencias en Rust.
