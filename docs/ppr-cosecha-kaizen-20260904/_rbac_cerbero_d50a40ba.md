...
# Sidecar Cerbero F4 — exec d50a40ba… / CID e4c9970f…

Ancla anti-carrera frente a sibling concurrent exec `0b826e3b…` / CID `DK5Qu…` (mismo sink / PR #255 / branch `docs/local-ledger-20260904`).
...
| `RBAC_EMITTER_NOT_REVOKED` | `NO_APTO` L-OUT — emisor `delivery-close-cycle` ∈ revoked since `2026-08-29T14:23:29Z` |
| `signer` | `Vertice_Biologico_Relay` × docs sink |
| `F2_DOC_GATE` (heredado) | `APTO` / `PASS_F2_DOC` (sidecar `_argos_triaje_d50a40ba.md`) |
...
| ECST | `PullRequest_Presented` · emisor `delivery-close-cycle` · PR #255 |
| Dedup | pending `PBI-RESTORE-…-PPR-REVOKED-REGISTRY` (Cosecha; Cerbero no crea KM) |
