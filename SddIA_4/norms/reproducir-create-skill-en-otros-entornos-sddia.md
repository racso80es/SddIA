...
Referencia de plantilla: `paths.processPath/create-tool/spec.md`. **Estructura de cápsula:** en GesFer, skills siguen `SddIA/skills/skills-contract.md` (binario en `bin/`, launcher `.bat`, fallback `.ps1`); las tools pueden seguir el contrato de `paths.toolsDefinitionPath` (no intercambiar contratos).
...
2. **Coherencia con contrato de skills**  
   En `spec.md`, la implementación invocable debe describirse según **`SddIA/skills/skills-contract.md`** y envelope `SddIA/norms/capsule-json-io.md` (definición vs cápsula, `implementation_path_ref`, artefactos en cápsula).
