...
description: >-
  ...
  produce informe de consenso y semillas Kaizen en paths.tasksPath (entrada tasks_path_cumulo).
inputs:
  ...
  - description: Documentación SddIA, tickets o READMEs relevantes para el alcance del cambio.
    ...
    type: string
  - description: >-
      Directorio destino para tareas Kaizen generadas; debe coincidir con paths.tasksPath del Cúmulo
      (misma ruta canónica: consultar SddIA/agents/cumulo.paths.json → paths.tasksPath).
...
outputs:
  ...
  - description: Informe de consenso multi-agente y veredicto (frontmatter YAML + Markdown); cumple rol de validación del ciclo.
    name: validacion.md
    type: file
  - description: Semillas Kaizen opcionales; un fichero por tema en paths.tasksPath (entrada tasks_path_cumulo).
    name: '[YYYYMMDD]-Refactor-[Tema].md'
    type: file_pattern
persist_ref: paths.featurePath/validate-pull-requests-<pr-slug>
phases:
  - description: >-
      Ejecutar git-workspace-recon para validar entorno limpio y coherencia local. A continuación usar git-branch-manager
      (u orquestación equivalente autorizada) para alinear el working tree con la rama origen del PR. Verificar contexto
      Karma2Token según contrato (paths.tokensPath). Confirmar que el análisis se aplicará al código propuesto en el PR,
      no a la rama de integración destino. Norma: SddIA/norms/git-via-skills-or-process.md.
    id: '0'
    name: Preparar contexto y rama del PR
  - description: >-
      Escrutinio de alineación y estructura (Clean Architecture, separación de responsabilidades, modularidad,
      ecosistema de nombres). Agente SddIA/agents/architect.json (System Architect).
    id: '1'
    name: Escrutinio architect
  - description: >-
      Escrutinio de necesidad, ausencia de alucinación (APIs/símbolos existentes en la rama), estados frontera y
      rendimiento. Agente SddIA/agents/qa-judge.json.
    id: '2'
    name: Escrutinio qa-judge
  - description: >-
      Escrutinio de inmunidad (exposición de datos, sanitización, vulnerabilidades). Hallazgos bloqueantes.
  ...
  - description: >-
      Persistir validacion.md bajo persist_ref. Escribir semillas Kaizen en tasks_path_cumulo (paths.tasksPath) con
      nombre [YYYYMMDD]-Refactor-[Tema].md y plantilla indicada en este documento. Consolidar hitos atómicos con
      git-save-snapshot cuando el flujo versiona la rama de revisión. Ante fallo estructural del entorno, valorar
      git-tactical-retreat según política. Cierre de publicación: ejecutar git-sync-remote y, si procede un PR de
      acompañamiento o actualización documental, git-create-pr incorporando al cuerpo objectives.md y validacion.md
      (y referencia al PR objeto de revisión).
    id: '5'
...
Este documento define el **proceso de tarea** para revisar un Pull Request con criterio **S+ Grade**. Está ubicado en `paths.processPath/validate-pull-requests/` (Cúmulo). Las rutas de persistencia de la revisión y las semillas Kaizen se obtienen de **Cúmulo** (`paths.featurePath`, `paths.tasksPath` vía entrada `tasks_path_cumulo`, `paths.tokensPath`).
...
1. **Sincroniza** el contexto de trabajo con esa rama: **git-workspace-recon** y **git-branch-manager** (u orquestación equivalente autorizada; ver `SddIA/norms/git-via-skills-or-process.md`).
...
- **Documentación de la tarea de revisión:** Cúmulo (`paths.featurePath/validate-pull-requests-<pr-slug>/`). El `<pr-slug>` se deriva de `pr_path_or_id` (identificador legible y seguro para nombre de carpeta; sin caracteres de ruta).
- **Semillas Kaizen (refactors diferidos):** directorio indicado por `tasks_path_cumulo`, que debe resolverse a **`paths.tasksPath`** del contrato Cúmulo (`SddIA/agents/cumulo.paths.json`). Nombre de fichero: `[YYYYMMDD]-Refactor-[Tema].md` (fecha según el día de creación de la semilla; en este repositorio, año de referencia operativa: usar la fecha actual del entorno de ejecución).

**Compatibilidad con otros procesos:** el proceso `automatic_task` puede consumir tareas Kaizen desde la raíz de `paths.tasksPath` o desde `paths.tasksPath/KAIZEN/` (ver `paths.processPath/automatic_task/spec.md`). Si el equipo prioriza la cola Kaizen explícita, las semillas pueden copiarse o generarse también bajo `paths.tasksPath/KAIZEN/` según convención interna, sin sustituir el contrato de nombre del presente proceso.

## Fases del proceso

| Fase | Nombre | Descripción |
|:-----|:-------|:------------|
| 0 | Preparar contexto y rama del PR | **git-workspace-recon**; **git-branch-manager** (o equivalente autorizado) hacia la rama del PR; Karma2Token; confirmación de alcance. |
...
| 5 | Persistencia y Cúmulo Kaizen | `objectives.md` y `validacion.md` en `persist_ref`; Kaizen en `tasks_path_cumulo`; **git-save-snapshot** en hitos; **git-tactical-retreat** si aplica; cierre con **git-sync-remote** y **git-create-pr** cuando se publique documentación o ajustes en rama propia. |
...
Si se han detectado mejoras no bloqueantes, generar el contenido en Markdown para `tasks_path_cumulo` (resuelto a `paths.tasksPath`).
...
- **objectives.md:** `process_id: validate-pull-requests`, `pr_path_or_id`, `pr_branch_name`, `date`, `status`, confirmación explícita de sincronización con la rama del PR (y referencia a Karma2Token si aplica).
- **validacion.md:** `process_id: validate-pull-requests`, `action_id: validate`, `feature_id` o carpeta (`validate-pull-requests-<pr-slug>`), `date`, `status`, `verdict` (aprobado | requiere_cambios | rechazado).
...
* **Arquitecto, QA-Judge, Security-Engineer:** Cargar definiciones desde `paths.agentsPath` según la tabla de identidad (refs en frontmatter).
* **Cúmulo:** Validar rutas vía `paths-via-cumulo`; `tasks_path_cumulo` → `paths.tasksPath` para semillas Kaizen.
* **Principios:** Coherencia `paths.principlesPath` (principles-contract) en diseño cuando aplique.

## Referencias

* Contrato de procesos: `paths.processPath/process-contract.md` y `paths.processPath/process-contract.json`
